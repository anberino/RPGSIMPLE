...
Some time later, after talking with some friends about a JoJo's RPG, I stumbled upon Esby's system and I was astonished by how short and comprehensive it was. Everything just made sense and it felt pretty dynamic to play with. So, I decided I would adapt it for our next campaign towards a medieval setting and see how it could work out. At some point during that adaptation, I realized it would be a lot more simple if I could just have a system that is easy to adapt, instead of multiple easy to play systems that took a lot of time to create. From that, I decided to create a new, simple and adaptable system, that would serve as a framework for all my future campaigns with my playgroup.
...
Simple is heavily based of ["JoJo's Bizarre Adventure - The RPG", by Esby](https://is.gd/JoJoRPG). The credit for creating a balanced and functional RPG system goes to Esby, while Simple tries only to adapt it so that it can be used in worlds and scenarios outside of those from JoJo's Bizarre Adventure. If for some weird yet maybe understandable reason you decided to use Simple for your JoJo's campaign, please use Esby's system and its expansions instead, as it is much more complete.
...
When creating a character in Simple, the first thing you'll want to do is to establish their basic characteristics. The first one is their "name". The top of the character sheet contains a slot for the player's name, the character's name and the character's level. All characters should start at level 1. Things like "age", "gender", "height" and "weight" are often things you make up right away, however these should be written down in the "Physical Description" box in the back of the sheet.

The next thing you'll see on the character sheets are "Traits". Traits are certain world-dependant aspects that every character has and are used to differentiate characters directly during an RPG experience. A Trait often represents a set of characteristics shared by all characters with said Trait. Things like a character's "Race", "Class", "Job", "Place of origin", "Color of mana", "School of thought" and so on. These can often indicate special abilities a character has, different ways other characters interact to them or even different ways a player should play their character. As a GM, your job is to create the Traits players can choose from and plan how each one will work in the world the adventure takes place.

*__Example:__ A traditional medieval RPG made in Simple could have two traits: "Race" and "Class". In the "Race" trait, players could choose from "Human", "Elf", "Dwarf" and "Halfling", while in the class trait they could choose from "Berserker", "Ranger", "Rogue", "Wizard", "Bard" and "Paladin". These could provide starting abilities and bonuses when they level up, and also affect the way the characters interact with eachother. However, the same RPG could have a third trait slot, "Place of origin", in which players could choose between "Northern lands", "Southern lands", "Eastern lands" and "Western lands". This final trait wouldn't directly provide Abilities or Advantages to your character (more on that later!), but maybe characters from one place of origin could have rivalries with ones from another, independing from the rest of their traits. Maybe the players encountered a shop during your adventure where the owner would only sell wares to those from the same land as him, and other situations like that.*
...
As such, Simple handles "Species" and "Class" by these simply being codewords for a certain set of *Abilities* and *Advantages* (*AA* for short) that goes in a Trait slot. That means that if in your RPG world there are only humans, there is no need to add a "Species" slot for your players to fill in their character sheets. In the case that there are different species to chose from, each one refers to a set of *AA* that your character will start the game with - and maybe also a set of *AA* they may be able to learn later through leveling up. The same is applied to the "Classes".
...
An important note is that Traits do not necessarily cancel eachother out. Maybe in the world you created, Traits are just specific set of abilities a character can have and they are not separated in different categories among Traits. This means that you could simply have a pool of 20 Traits and each character can choose 2 of them, for example. If for some reason you decide you don't need characters to have Traits, you could leave their slots blank.
...
"Physical Description" applies to how your character looks, from hair color, to height, to weight (as mentioned before) and to how they dress in general. A character's physical description may influence what they wear and carry and also be influenced by their *Attributes* and *AAs*.
...
Esby for creating the awesome JoJo's RPG.
